    ...
    if len(card_id) != 10:
        ...
        for i in card_id[0:5]:
            if character_value(i) < 0 or character_value(i) > 25:
                return (False, 'The first 5 characters must be A-Z, the invalid character is at index ' + str(card_id.index(i)) + ' (value of ' + i + ')')
                break
        for i in card_id[7:10]:
            if i.isdigit() == False:
                return (False, 'The last 3 characters must be 0-9, the invalid character is at index ' + str(card_id.index(i)) + ' (value of ' + i + ')')
                break
        if get_school(card_id) == False:
            ...
            return (False, 'Check Digit ' + str(card_id[9]) + ' does not match calculated value ' + str(get_check_digit(card_id)) + '.')
        else:
            return (True, '')
